...
class _ViewCourseState extends State<ViewCourse> with SingleTickerProviderStateMixin{
  ...
  Widget courseGrps(){
    return FutureBuilder(
      future: fblink.getCourseGrps(_school, _courseID),
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot){
        if(!snapshot.hasData){
          ...
          children: snapshot.data.documents.map((DocumentSnapshot document){
            _moduleName = document.data["courseGrp"];
            return ListTile(
              ...
              trailing: new IconButton(
                ...
                onPressed: (){
                  confirmDelete(context, _moduleName,1);
                  setState(() {
                  });
                },
              ),
  ...
  Widget modules(){
    return FutureBuilder(
      future: fblink.getCourseModules(_school, _courseID),
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot){
        if(!snapshot.hasData){
          return Assets.loader();
        }else{
          ...
          children: snapshot.data.documents.map((DocumentSnapshot document){
            ...
            return ListTile(
              ...
              trailing: new IconButton(
                icon: Icon(Icons.delete),
                onPressed: (){
                  confirmDelete(context, _moduleName,0);
                  setState(() {
                  });
                },
              ),
  ...
  Future<Null> addCourseGrpDialog()async{
    ...
    switch(
      await showDialog(
        ...
        child: new SimpleDialog(
          contentPadding: EdgeInsets.all(15.0),
          title: new Text("Add Coure Group",textAlign: TextAlign.center,),
          children: <Widget>[
            new Logo(150.0,"lib/Assets/AddModule.png"),
            new Form(
              key: courseGrpKey,
            ...
            new FlatButton( 
               onPressed:((){
                ...
                switch(type){
                  ...
                  break;
                  case 1:
                  fblink.removeCourseGrp(_school,_courseID, moduleName);
                  break;
                }
                setState(() {
      ...
      try {
        ...
        try {
          switch(type){
            ...
            break;
            case 1:{
              await fbLink.addCourseGrps(_school,_courseID,_courseGrp).then((CourseCreationResult ccr){
                if(ccr.success){
                  _authHint = ccr.error;
                  final form = key.currentState;
                  setState(() {
